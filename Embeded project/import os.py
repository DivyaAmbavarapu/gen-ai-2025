...
class TextClassifier:
    ...
    def preprocess_data(self, file_path):
        ...
        rows = []
        # Improved regex: matches Ticket #001 - System Crash, Ticket #002: Software Failure, etc.
        ...
        for ticket_num, issue_text in ticket_pattern:
            ...
            if "database" in issue_text_lower:
                y_label = "database crash"
            elif "software" in issue_text_lower or "application" in issue_text_lower:
                y_label = "software crash"
            elif "server" in issue_text_lower or "power supply" in issue_text_lower:
                y_label = "server crash"
            elif any(k in issue_text_lower for k in ["system", "cpu", "memory", "blue screen", "thermal"]):
                y_label = "system crash"
            elif any(k in issue_text_lower for k in ["network", "connection", "nic", "ethernet"]):
                y_label = "network issue"
            else:
                y_label = "other issue"

            rows.append({"ticket_number": f"Ticket #{ticket_num}", "x_text": issue_text, "y_label": y_label})
